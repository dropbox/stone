...
class TestGeneratedPythonTypes(unittest.TestCase):
    ...
    def _mock_backend(self) -> PythonTypesBackend:
    ...
    def _evaluate_struct(self, ns: ApiNamespace, struct: Struct) -> typing.Text:
        backend = self._mock_backend()
        backend._generate_struct_class(ns, struct)
        return backend.output_buffer_to_string()

    def _evaluate_annotation_type(
    ...
    def test_struct_with_custom_annotations(self) -> None:
        ns = ApiNamespace("files")
        ...
        annotation = CustomAnnotation(
            "MyAnnotation", ns, None, "MyAnnotationType", None, [], {"test_param": 42}
        )
    ...
    def test_annotation_type_class(self) -> None:
        ns = ApiNamespace("files")
        annotation_type = AnnotationType(
            "MyAnnotationType",
            ns,
            ...
            [
                AnnotationTypeParam(
                    "test_param", Int32(), "test parameter", False, None, None,
                ),
                AnnotationTypeParam(
                    "test_default_param", Int32(), None, True, 5, None,
                ),
            ],
        )
