...
from stone.ir import ApiNamespace, Boolean, Struct, StructField


def _make_backend(
    target_folder_path: typing.Text, template_path: typing.Text
) -> TSDTypesBackend:

    args = Mock()
    args.__iter__ = Mock(return_value=iter([template_path, "-i=0"]))

    return TSDTypesBackend(target_folder_path=str(target_folder_path), args=args)


def _make_namespace(ns_name: typing.Text = "accounts") -> ApiNamespace:
...
def _make_struct(
    struct_name: typing.Text, struct_field_name: typing.Text, namespace: ApiNamespace
) -> Struct:
...
def _evaluate_namespace(
    backend: TSDTypesBackend, namespace_list: typing.List[ApiNamespace]
) -> typing.Text:

    get_result = _mock_output(backend)
    filename = "types.d.ts"
    backend.split_by_namespace = False
    backend._generate_base_namespace_module(
...
class TestTSDTypes(unittest.TestCase):
    ...
    def test__generate_types_single_ns(self) -> None:
    ...
    def test__generate_types_empty_ns(self) -> None:
        backend = _make_backend(target_folder_path="output", template_path="")
    ...
    def test__generate_types_with_empty_ns(self) -> None:
        backend = _make_backend(target_folder_path="output", template_path="")
        ns = _make_namespace()
        empty_ns = ApiNamespace("empty_namespace")
    ...
    def test__generate_types_multiple_ns(self) -> None:
        backend = _make_backend(target_folder_path="output", template_path="")
...
"""
    ...
    def get_ns_types_as_declaration(cls):
        types = (
            ...
            + (
                ("\ndeclare module 'ns' " + cls._ns_spec_types)
...
class TestTSDTypesE2E(unittest.TestCase):
    ...
    def setUp(self):
        ...
        template_file_path = f"{self.stone_output_directory}/{self.template_file_name}"
    ...
    def test_tsd_types_namespace_output(self):
        ...
        self._verify_generated_output(f"output/{output_file_name}", expected_output)
