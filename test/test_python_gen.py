...
from stone.backends.python_rsrc.stone_serializers import CallerPermissionsInterface
from stone.backends.python_rsrc.stone_serializers import _strftime as stone_strftime
from stone.backends.python_rsrc.stone_serializers import json_decode, json_encode
...
class TestDropInModules(unittest.TestCase):
    ...
    def mk_validator_testers(self, validator):
        ...
        return (
            p,
            f,
        )  # 'p(input)' if you expect it to pass, 'f(input)' if you expect it to fail.
    ...
    def test_float_validator(self):
        ...
        self.assertRaises(
            AssertionError, lambda: bv.Float64(min_value=0, max_value=10 ** 330)
        )
    ...
    def test_timestamp_validator(self):
        class UTC(datetime.tzinfo):
            def utcoffset(
                self, dt
            ):  # pylint: disable=unused-argument,useless-suppression
        ...
        self.assertRaises(
            bv.ValidationError, lambda: t.validate(now.replace(tzinfo=PST()))
        )
    ...
    def test_nullable_validator(self):
        ...
        self.assertRaises(AssertionError, lambda: bv.Nullable(bv.Nullable(bv.String())))
    ...
    def test_json_encoder(self):
        ...
        b = b"\xff" * 5
        self.assertEqual(
            json_encode(bv.Bytes(), b), json.dumps(base64.b64encode(b).decode("ascii"))
        )
        self.assertEqual(json_encode(bv.Nullable(bv.String()), None), json.dumps(None))
        self.assertEqual(
            json_encode(bv.Nullable(bv.String()), "abc"), json.dumps("abc")
        )
    ...
    def test_json_encoder_union(self):
        ...
        u = U("a", 64)
        self.assertEqual(
            json_encode(bv.Union(U), u, old_style=True), json.dumps({"a": 64})
        )
        ...
        u = U("c", c)
        self.assertEqual(
            json_encode(bv.Union(U), u, old_style=True),
        ...
        u = U("d", [1, 2, 3, 4])
        self.assertEqual(
            json_encode(bv.Union(U), u, old_style=True), json.dumps({"d": l1})
        )

        # Test a nullable union
        self.assertEqual(json_encode(bv.Nullable(bv.Union(U)), None), json.dumps(None))
        self.assertEqual(
            json_encode(bv.Nullable(bv.Union(U)), u, old_style=True),
            json.dumps({"d": l1}),
        )
        ...
        u = U("e", None)
        self.assertEqual(
            json_encode(bv.Nullable(bv.Union(U)), u, old_style=True), json.dumps("e")
        )
        u = U("e", 64)
        self.assertEqual(
            json_encode(bv.Nullable(bv.Union(U)), u, old_style=True),
        ...
        u = U("f", None)
        self.assertEqual(
            json_encode(bv.Nullable(bv.Union(U)), u, old_style=True), json.dumps("f")
        )
        u = U("f", c)
        self.assertEqual(
            json_encode(bv.Nullable(bv.Union(U)), u, old_style=True),
        ...
        u = U("g", m)
        self.assertEqual(
            json_encode(bv.Union(U), u, old_style=True), json.dumps({"g": m})
        )
    ...
    def test_json_decoder(self):
        ...
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.String(), json.dumps(32))
        )

        self.assertEqual(json_decode(bv.UInt32(), json.dumps(123)), 123)
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.UInt32(), json.dumps("hello"))
        )

        self.assertEqual(json_decode(bv.Boolean(), json.dumps(True)), True)
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.Boolean(), json.dumps(1))
        )
        ...
        self.assertEqual(
            json_decode(
                bv.Timestamp("%a, %d %b %Y %H:%M:%S +0000"), json.dumps(now.strftime(f))
            ),
            now,
        )
        ...
        b = b"\xff" * 5
        self.assertEqual(
            json_decode(bv.Bytes(), json.dumps(base64.b64encode(b).decode("ascii"))), b
        )
        self.assertRaises(
            bv.ValidationError, json_decode, bv.Bytes(), json.dumps("=non-base64=")
        )
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.Bytes(), json.dumps(1))
        )
    ...
    def test_json_decoder_struct(self):
        class S:
            ...
            _all_fields_ = [("f", bv.String()), ("g", bv.Nullable(bv.String()))]
            _g = None
        ...
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.Struct(S), json.dumps({}))
        )
        ...
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.Struct(S), msg, strict=True)
        )
    ...
    def test_json_decoder_union(self):
        ...
        u = json_decode(
            bv.Union(U), json.dumps({"b": [1, 2]}), strict=False, old_style=True
        )
        self.assertEqual(u._tag, "b")
        self.assertIsInstance(u._tag, str)
        ...
        self.assertRaises(
            bv.ValidationError, lambda: json_decode(bv.Union(U), json.dumps("z"))
        )
        ...
        u = json_decode(
            bv.Union(U), json.dumps({"z": "test"}), strict=False, old_style=True
        )
        ...
        # Test nullable union
        u = json_decode(
            bv.Nullable(bv.Union(U)), json.dumps(None), strict=False, old_style=True
        )
        ...
        u = json_decode(
            bv.Union(U), json.dumps({"e": 64}), strict=False, old_style=True
        )
        self.assertEqual(u._tag, "e")
        self.assertIsInstance(u._tag, str)
        ...
        self.assertIsInstance(u._tag, str)
        u = json_decode(
            bv.Union(U), json.dumps({"f": {"f": "hello"}}), strict=False, old_style=True
        )
...
class TestGeneratedPython(unittest.TestCase):
    def setUp(self):
        ...
        _, stderr = p.communicate(input=(test_spec + test_ns2_spec).encode("utf-8"))
        if p.wait() != 0:
            raise AssertionError(
                "Could not execute stone tool: %s" % stderr.decode("utf-8")
            )
    ...
    def test_struct_decoding(self):
        ...
        d = self.decode(
            bv.Struct(self.ns.D), json.dumps({"a": "A", "b": 1, "d": [], "e": {}})
        )
        self.assertEqual(d.a, "A")
        self.assertEqual(d.b, 1)
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.decode(bv.Struct(self.ns.D), json.dumps({"a": "A", "b": None}))
    ...
    def test_union_decoding_old(self):
        ...
        self.assertEqual(
            "'123' expected to be a string, got integer", str(cm.exception)
        )
    ...
    def test_union_decoding(self):
        ...
        # Test catch-all
        v = self.decode(
            bv.Union(self.ns.V), json.dumps({".tag": "z", "z": "hello"}), strict=False
        )
        self.assertIsInstance(v, self.ns.V)
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.decode(
                bv.Union(self.ns.V), json.dumps({".tag": "t2", "t2": None, "unk": 123})
            )
        self.assertEqual("unexpected key 'unk'", str(cm.exception))
        ...
        v = self.decode(bv.Union(self.ns.V), json.dumps({".tag": "t6"}))
        self.assertIsInstance(v, self.ns.V)
        ...
        v = self.decode(bv.Union(self.ns.V), json.dumps({".tag": "t8"}))
        self.assertIsInstance(v, self.ns.V)
        self.assertEqual(v.get_t8(), None)

        # Test member that is a list
        v = self.decode(
            bv.Union(self.ns.V), json.dumps({".tag": "t9", "t9": ["a", "b"]})
        )
        self.assertIsInstance(v, self.ns.V)
        ...
        v = self.decode(
            bv.Union(self.ns.V), json.dumps({"t10": [{"t1": "hello"}]}), old_style=True
        )
        self.assertIsInstance(v, self.ns.V)
        t10 = v.get_t10()
        self.assertEqual(t10[0].get_t1(), "hello")
        ...
        v = self.decode(
            bv.Union(self.ns.V), json.dumps({".tag": "t0", "t0": "hello"}), strict=False
        )
        self.assertIsInstance(v, self.ns.V)
        self.assertTrue(v.is_t0())
        ...
        # test maps
        v = self.decode(
            bv.Union(self.ns.V), json.dumps({".tag": "t11", "t11": {"a": 100}})
        )
        self.assertIsInstance(v, self.ns.V)
    ...
    def test_union_decoding_with_optional_struct(self):
        ...
        # fields).
        u2 = self.decode(bv.Union(self.ns.U2), json.dumps({".tag": "b"}))
    ...
    def test_union_equality_with_value(self):
        ...
        u_equal = self.decode(
            bv.Union(self.ns.U), json.dumps({".tag": "t1", "t1": "a"})
        )
        u_unequal = self.decode(
            bv.Union(self.ns.U), json.dumps({".tag": "t1", "t1": "b"})
        )
        self.assertEqual(u, u_equal)
        self.assertEqual(hash(u), hash(u_equal))
        self.assertNotEqual(u, u_unequal)
    ...
    def test_extended_union_equality(self):
        ...
        u = self.decode(bv.Union(self.ns.U), json.dumps({".tag": "t0"}))
        u_extend = self.decode(bv.Union(self.ns.UExtend), json.dumps({".tag": "t0"}))
        u_extend_extend = self.decode(
            bv.Union(self.ns.UExtendExtend), json.dumps({".tag": "t0"})
        )
    ...
    def test_struct_decoding_with_optional_struct(self):
        opt_s = self.decode(bv.Struct(self.ns.OptionalS), json.dumps(None))
        ...
        s2 = self.decode(bv.Struct(self.ns.S2), json.dumps({}))
    ...
    def test_struct_equality_with_different_types(self):
        ...
        # have a direct line of inheritance to each other.
        s_extend = self.decode(
            bv.Struct(self.ns.SExtend), json.dumps({"f": "F", "g": "G"})
        )
        s_extend2 = self.decode(
            bv.Struct(self.ns.SExtend2), json.dumps({"f": "F", "g": "G"})
        )
    ...
    def test_extended_struct_equality(self):
        ...
        exact same fields."""
        s = self.decode(bv.Struct(self.ns.S), json.dumps({"f": "F"}))
        s_extend_empty = self.decode(
            bv.Struct(self.ns.SExtendEmpty), json.dumps({"f": "F"})
        )
        s_extend = self.decode(
            bv.Struct(self.ns.SExtend), json.dumps({"f": "F", "g": "G"})
        )
    ...
    def test_union_encoding(self):
        ...
        v_t0 = self.ns.V.t0
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t0), {".tag": "t0"}
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t1),
            {".tag": "t1", "t1": "hello"},
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t2),
            {".tag": "t2", "t2": "hello"},
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t2), {".tag": "t2"}
        )
        ...
        v_t3 = self.ns.V.t3(s)
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t3),
            {".tag": "t3", "f": "hello"},
        )
        ...
        v_t4 = self.ns.V.t4(s)
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t4),
            {".tag": "t4", "f": "hello"},
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t4), {".tag": "t4"}
        )

        # Test stacked unions
        v_t5 = self.ns.V.t5(self.ns.U.t1("hello"))
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t5),
        ...
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t6), {".tag": "t6"}
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(bv.Union(self.ns.V), v_t8), {".tag": "t8"}
        )
    ...
    def test_list_coding(self):
        ...
        v = self.decode(bv.List(bv.Struct(self.ns.S)), json.dumps([{"f": "Test"}]))
        self.assertIsInstance(v, list)
        self.assertIsInstance(v[0], self.ns.S)
        self.assertEqual(v[0].f, "Test")

        # Test encoding list of composites
        v = self.encode(bv.List(bv.Struct(self.ns.S)), [self.ns.S("Test")])
    ...
    def test_struct_enumerated_subtypes_decoding(self):
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_decode(
                bv.StructTree(self.ns.Resource), {".tag": "unk", "name": "test.doc"}
            )
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_decode(
                bv.StructTree(self.ns.Resource), {".tag": 123, "name": "test.doc"}
            )
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_decode(
                bv.StructTree(self.ns.Resource), {".tag": "symlink", "name": "test"}
            )
    ...
    def test_msgpack(self):
        ...
        try:
            from stone_serializers import msgpack_decode, msgpack_encode
        except ImportError:
            return
    ...
    def test_alias_validators(self):
        ...
        aliased_validators = {self.ns.AliasedString_validator: aliased_string_validator}
...
class TestCustomStrftime(unittest.TestCase):
    def test_strftime(self):
        s = stone_strftime(
            datetime.date(1800, 9, 23), "%Y has the same days as 1980 and 2008"
        )
...
class TestAnnotationsGeneratedPython(unittest.TestCase):
    def setUp(self):
        ...
        _, stderr = p.communicate(
            input=(
                test_tagged_spec + test_tagged_patched_spec + test_tagged_spec_2
            ).encode("utf-8")
        )
        if p.wait() != 0:
            raise AssertionError(
    ...
    def test_struct_child_encoding(self):
        ...
        ai = self.ns3.A(
            ...
            b=1,
            c="C",
            d=[self.ns3.X(a="A", b="B")],
            e={},
            f=self.ns3.X(a="A", b="B"),
            g=4,
        )

        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.A), ai, caller_permissions=self.internal_and_alpha_cp
            ),
            json_data,
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.A), ai, caller_permissions=self.default_cp
            ),
            json_data,
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.A), ai, caller_permissions=self.alpha_cp
            ),
            json_data,
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.A), ai, caller_permissions=self.internal_cp
            ),
            json_data,
        )
    ...
    def test_struct_parent_encoding(self):
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.B), bi, caller_permissions=self.internal_and_alpha_cp
            ),
            json_data,
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.B), bi, caller_permissions=self.default_cp
            ),
            json_data,
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.B), bi, caller_permissions=self.alpha_cp
            ),
            json_data,
        )
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.B), bi, caller_permissions=self.internal_cp
            ),
            json_data,
        )
    ...
    def test_union_closed_parent_encoding(self):
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Union(self.ns3.U), ui, caller_permissions=self.internal_and_alpha_cp
            ),
            json_data,
        )
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_encode(
                bv.Union(self.ns3.U), ui, caller_permissions=self.default_cp
            )
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_encode(
                bv.Union(self.ns3.U), ui, caller_permissions=self.internal_cp
            )
        self.assertEqual("caller does not have access to 't2' tag", str(cm.exception))
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Union(self.ns3.U), ui, caller_permissions=self.alpha_cp
            ),
            json_data,
        )
    ...
    def test_union_open_child_encoding(self):
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_encode(
                bv.Union(self.ns3.UOpen), ui, caller_permissions=self.default_cp
            )
        self.assertEqual("caller does not have access to 't1' tag", str(cm.exception))
        ...
        with self.assertRaises(bv.ValidationError) as cm:
            self.compat_obj_encode(
                bv.Union(self.ns3.UOpen), ui, caller_permissions=self.internal_cp
            )
        self.assertEqual("caller does not have access to 't2' tag", str(cm.exception))
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.Union(self.ns3.UOpen), ui, caller_permissions=self.alpha_cp
            ),
            json_data,
        )
    ...
    def test_enumerated_subtype_encoding(self):
        ...
        # test full super-type
        fi = self.ns3.File(name="File1", size=5, x=self.ns3.X(a="A", b="B"), y="Y")
        ...
        fi = self.ns3.File(name="File1", size=5, y="Y")
        ...
        fi = self.ns3.File(name="File1", size=5, x=self.ns3.X(a="A", b="B"))
        ...
        self.assertEqual(
            self.compat_obj_encode(
                bv.StructTree(self.ns3.File), fi, caller_permissions=self.default_cp
            ),
            json_data,
        )
    ...
    def test_struct_parent_encoding_with_redaction(self):
        json_data = {
            "a": "A",
            "b": "e4c2e8edac362acab7123654b9e73432",
            "c": "********",
            "d": [
                {
                    "a": "***-blot-***",
                    "b": "3ac5041d7a9d0f27e045f0b15034f186 (***-hash-***)",
                }
            ],
            "e": {"e1": "********"},
            "f": {
        ...
        ai = self.ns3.A(
            ...
            b=1,
            c="C",
            d=[self.ns3.X(a="TEST-blot-TEST", b="TEST-hash-TEST")],
            e={"e1": "e2"},
            f=self.ns3.X(a="TEST-blot-TEST", b="TEST-hash-TEST"),
            g=4,
        )
    ...
    def test_union_closed_parent_encoding_with_redaction(self):
        ...
        json_data = {".tag": "t1", "t1": "c983987c3b0629b9906c5c7d353409fe"}
        ...
        json_data = {
            ".tag": "t2",
            "t2": [{"a": "********", "b": "9d5ed678fe57bcca610140957afab571"}],
        }
        ...
        json_data = {
            ".tag": "t3",
            "t3": [{"a": "********", "b": "9d5ed678fe57bcca610140957afab571"}],
        }
    ...
    def test_encoding_collections_with_redaction(self):
        ...
        json_data = {
            "a": ["********"],
            "b": {"key": "74e710825309d622d0b920390ef03edf"},
        }

        s = self.ns3.S2(a=["test_str"], b={"key": "test_str"})
        self.assertEqual(
            self.compat_obj_encode(
                bv.Struct(self.ns3.S2),
                s,
                caller_permissions=self.internal_and_alpha_cp,
                should_redact=True,
            ),
    ...
    def test_encoding_unicode_with_redaction(self):
        ...
        json_data = {
            ".tag": "t2",
            "t2": [{"a": "********", "b": "89e514e90912003ff11d79560a750510"}],
        }
