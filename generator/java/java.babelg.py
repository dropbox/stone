...
class JavaCodeGenerator(CodeGenerator):
        ...
        with self.block():
            ...
            with self.block():
                args = [camelcase(field.name) for field
                        in route.request_data_type.all_fields]
                # This line uses the existing method that takes all possible arguments
                # for brevity. When we no longer expose that method then we can
                # generate the code here instead.
                out('%s%s.this.%s(%s);' % (ret, outer, method_name, ', '.join(args)))
    ...
    def generate_route_stuff(self, namespace, route, outer):
        ...
        if is_struct_type(route.request_data_type):
            ...
            if len(route.request_data_type.all_optional_fields) != 0:
                ...
                # just offer the two overloaded methods.
                if len(route.request_data_type.all_optional_fields) >= 2:
