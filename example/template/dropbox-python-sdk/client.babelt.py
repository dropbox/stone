...
{% for data_type in api.namespaces.v1.data_types %}
...
class {{ data_type.name|class }}({% if data_type.super_type %}{{ data_type.super_type.name|class }}{% else %}object{% endif %}):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """
        Creates a new {{ data_type.name|class }} object.
    ...
    def __repr__(self):
        s = ', '.join([
            {% for field in data_type.all_fields %}
                {% if field.data_type.name != "List" %}
                repr(self.{{ field.name }}),
                {% endif %}
            {% endfor %}
            ])
        return '{{ data_type.name|class }}(%s)' % s
...
"""
        ...
        Returns
            {{ op.name|class }}Response object.
        """
{%- endmacro %}

{% macro add_path(op) -%}
{% if op.request_segmentation.segments[0].data_type.fields and op.request_segmentation.segments[0].data_type.fields[0].name == 'path' %} + '/' + path{% endif %}
{%- endmacro %}
