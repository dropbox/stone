...
class Lexer:
        ...
        if self.tokens_queue:
            ...
            if isinstance(r, MultiToken):
                ...
                if r is None and self.cur_indent > 0:
                    if self.last_token and self.last_token.type not in (
                        "NEWLINE",
                        "LINE",
                    ):
    ...
    # List of token names
    tokens: typing.Tuple[typing.Text, ...] = (
        "ID",
        "KEYWORD",
        "PATH",
        "DOT",
    ...
    def t_ANY_STRING(self, t):
        ...
        lines_without_indentation = [
            line.replace(indentation_str, "", 1) for line in new_str.splitlines()
        ]
    ...
    def t_INITIAL_comment(self, token):
        ...
        while i >= 0:
            ...
            is_partial_line_comment = (
                not is_full_line_comment and token.lexer.lexdata[i] != " "
            )
    ...
    def t_ANY_error(self, token):
        self._logger.debug(
            "Illegal character %r at line %d", token.value[0], token.lexer.lineno
        )
        self.errors.append(
            (
