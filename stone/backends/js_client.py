...
_cmdline_parser.add_argument(
    ...
    type=str,
    help=(
        "The name of the class the generated functions will be attached to. "
    ...
    def _generate_route(self, route_schema, namespace, route):
        ...
        if route.doc:
            self.emit_wrapped_text(
                self.process_doc(route.doc, self._docf), prefix=" * "
            )
        if self.args.class_name:
            self.emit(f" * @function {self.args.class_name}#{function_name}")
        if route.deprecated:
            self.emit(" * @deprecated")

        self.emit(
            " * @arg {%s} arg - The request parameters." % fmt_type(route.arg_data_type)
        )
        self.emit(
            " * @returns {Promise.<%s, %s>}"
            % (fmt_type(route.result_data_type), fmt_error_type(route.error_data_type))
        )
        self.emit(" */")
