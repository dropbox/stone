...
from stone.ir import is_struct_type, is_union_type, is_void_type
...
_cmdline_parser.add_argument(
    ...
    type=str,
    help=(
        "The name of the Swift module to generate. Please exclude the .swift "
        "file extension."
    ),
)
_cmdline_parser.add_argument(
    ...
    type=str,
    help=(
        "The name of the Swift class that contains an object for each namespace, "
...
class SwiftBackend(SwiftBaseBackend):
    ...
    def generate(self, api):
        for namespace in api.namespaces.values():
            ...
            if namespace.routes:
                with self.output_to_relative_path(f"{ns_class}Routes.swift"):
                    self._generate_routes(namespace)

        with self.output_to_relative_path(f"{self.args.module_name}.swift"):
    ...
    def _generate_client(self, api):
        ...
        with self.block(f"open class {self.args.class_name}"):
            namespace_fields = []
            for namespace in api.namespaces.values():
                if namespace.routes:
                    namespace_fields.append((namespace.name, fmt_class(namespace.name)))
            for var, typ in namespace_fields:
                self.emit(
                    "/// Routes within the {} namespace. "
                    "See {}Routes for details.".format(var, typ)
                )
                self.emit(f"open var {var}: {typ}Routes!")
            self.emit()

            with self.function_block(
                "public init",
                args=self._func_args(
                    [("client", f"{self.args.transport_client_name}")]
                ),
            ):
                for var, typ in namespace_fields:
                    self.emit(f"self.{var} = {typ}Routes(client: client)")
    ...
    def _generate_routes(self, namespace):
        ...
        self.emit(f"/// Routes for the {namespace.name} namespace")

        with self.block(f"open class {ns_class}Routes"):
            self.emit(f"public let client: {self.args.transport_client_name}")
            args = [("client", f"{self.args.transport_client_name}")]
    ...
    def _get_route_args(self, namespace, route):
        ...
        if is_struct_type(data_type):
            ...
            doc_list = [
                (
                    fmt_var(f.name),
                    self.process_doc(f.doc, self._docf) if f.doc else undocumented,
                )
    ...
    def _emit_route(
        self, namespace, route, req_obj_name, extra_args=None, extra_docs=None
    ):
        ...
        output = output.format(
            fmt_type(route.result_data_type), fmt_type(route.error_data_type)
        )
        ...
        with self.function_block(
            f"@discardableResult open func {func_name}",
            args=self._func_args(arg_list, force_first=False),
            return_type=f"{req_obj_name}<{rtype}, {etype}>",
        ):
            ...
            if is_struct_type(route.arg_data_type):
                ...
                func_args += [
                    ("serverArgs", "{}({})".format(arg_type, self._func_args(args)))
                ]
                self.emit(
                    "let serverArgs = {}({})".format(arg_type, self._func_args(args))
                )
            elif is_union_type(route.arg_data_type):
                self.emit(
                    "let serverArgs = {}".format(fmt_var(route.arg_data_type.name))
                )
    ...
    def _maybe_generate_deprecation_warning(self, route):
        if route.deprecated:
            ...
            self.emit(f'@available(*, unavailable, message:"{msg}")')
    ...
    def _generate_route(self, namespace, route):
        ...
        if route_type not in client_args.keys():
            ...
            for args_data in client_args[route_type]:
                ...
                extra_docs = [
                    (type_data[0], type_data[-1]) for type_data in type_data_list
                ]
