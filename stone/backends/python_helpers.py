...
from stone.backends.helpers import fmt_pascal, fmt_underscores
...
@contextmanager
def emit_pass_if_nothing_emitted(codegen: CodeBackend) -> typing.Iterator[None]:
...
def generate_imports_for_referenced_namespaces(
    ...
    for ns in imported_namespaces:
        backend.emit(
            "from {package} import {namespace_name}{type_ignore_comment}".format(
                package=package,
...
def prefix_with_ns_if_necessary(
    name: typing.Text, name_ns: ApiNamespace, source_ns: ApiNamespace
) -> typing.Text:
    """
...
def class_name_for_data_type(data_type, ns=None):
    ...
    """
    assert is_user_defined_type(data_type) or is_alias(
        data_type
