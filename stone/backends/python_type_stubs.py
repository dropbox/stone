...
class ImportTracker:
    def __init__(self) -> None:
        self.cur_namespace_typing_imports: typing.Set[typing.Text] = set()
        self.cur_namespace_adhoc_imports: typing.Set[typing.Text] = set()

    def clear(self) -> None:
        self.cur_namespace_typing_imports.clear()
        self.cur_namespace_adhoc_imports.clear()

    def _register_typing_import(self, s: typing.Text) -> None:
        """
    ...
    def _register_adhoc_import(self, s: typing.Text) -> None:
        """
...
class PythonTypeStubsBackend(CodeBackend):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._pep_484_type_mapping_callbacks = (
            self._get_pep_484_type_mapping_callbacks()
        )
    ...
    def generate(self, api: Api) -> None:
        ...
        for namespace in api.namespaces.values():
            with self.output_to_relative_path(
                "{}.pyi".format(fmt_namespace(namespace.name))
            ):
                self._generate_base_namespace_module(namespace)

    def _generate_base_namespace_module(self, namespace: ApiNamespace) -> None:
    ...
    def _generate_typevars(self) -> None:
        """
    ...
    def _generate_annotation_type_class_properties(
        ...
        for param in annotation_type.params:
            ...
            self.emit("@property")
            self.emit(
                "def {prop_name}(self) -> {param_type}: ...".format(
                    prop_name=prop_name, param_type=param_type,
                )
            )
            self.emit()
    ...
    def _generate_validator_for(self, data_type: DataType) -> None:
        cls_name = class_name_for_data_type(data_type)
        self.emit(f"{cls_name}_validator: bv.Validator = ...")
    ...
    def _generate_union_class_vars(self, ns: ApiNamespace, data_type: Union) -> None:
        lineno = self.lineno
    ...
    def _generate_union_class_is_set(self, union: Union) -> None:
        for field in union.fields:
            field_name = fmt_func(field.name)
            self.emit(f"def is_{field_name}(self) -> bool: ...")
            self.emit()

    def _generate_union_class_variant_creators(
        self, ns: ApiNamespace, data_type: Union
    ) -> None:
        """
        ...
        for field in data_type.fields:
            if not is_void_type(field.data_type):
                ...
                self.emit(
                    "def {field_name}(cls, val: {val_type}) -> {union_type}: ...".format(
    ...
    def _generate_union_class_get_helpers(
        self, ns: ApiNamespace, data_type: Union
    ) -> None:
        """
        ...
        for field in data_type.fields:
            ...
            if not is_void_type(field.data_type):
                ...
                self.emit(
                    "def get_{field_name}(self) -> {val_type}: ...".format(
                        field_name=field_name, val_type=val_type,
                    )
                )
                self.emit()

    def _generate_alias_definition(self, namespace: ApiNamespace, alias: Alias) -> None:
    ...
    def _class_declaration_for_type(
        self, ns: ApiNamespace, data_type: typing.Union[Struct, Union]
    ) -> typing.Text:
    ...
    def _generate_struct_class_init(self, ns: ApiNamespace, struct: Struct) -> None:
        args = ["self"]
        for field in struct.all_fields:
            ...
            args.append(
                "{field_name}: {field_type} = ...".format(
                    field_name=field_name_reserved_check, field_type=field_type
                )
            )

        self.generate_multiline_list(args, before="def __init__", after=" -> None: ...")

    def _generate_struct_class_properties(
        self, ns: ApiNamespace, struct: Struct
    ) -> None:
        to_emit: typing.List[typing.Text] = []
        for field in struct.all_fields:
            field_name_reserved_check = fmt_func(field.name, check_reserved=True)
            field_type = self.map_stone_type_to_pep484_type(ns, field.data_type)

            to_emit.append(
                f"{field_name_reserved_check}: bb.Attribute[{field_type}] = ..."
            )
    ...
    def _get_pep_484_type_mapping_callbacks(self) -> OverrideDefaultTypesDict:
        """
        ...
        def upon_encountering_list(
            ns: ApiNamespace,
            data_type: DataType,
            override_dict: OverrideDefaultTypesDict,
        ) -> typing.Text:
        ...
        def upon_encountering_map(
            ns: ApiNamespace,
            map_data_type: DataType,
            override_dict: OverrideDefaultTypesDict,
        ) -> typing.Text:
            map_type = typing.cast(Map, map_data_type)
            self.import_tracker._register_typing_import("Dict")
            return "Dict[{}, {}]".format(
                map_stone_type_to_python_type(
                    ns, map_type.key_data_type, override_dict
                ),
                map_stone_type_to_python_type(
                    ns, map_type.value_data_type, override_dict
                ),
            )

        def upon_encountering_nullable(
            ns: ApiNamespace,
            data_type: DataType,
            override_dict: OverrideDefaultTypesDict,
        ) -> typing.Text:
        ...
        def upon_encountering_timestamp(
            ns: ApiNamespace,
            data_type: DataType,
            _override_dict: OverrideDefaultTypesDict,
        ) -> typing.Text:
            self.import_tracker._register_adhoc_import("import datetime")
            return map_stone_type_to_python_type(ns, data_type)

        def upon_encountering_string(_ns, _data_type, _override_dict) -> typing.Text:
            self.import_tracker._register_typing_import("Text")
            return "Text"

        callback_dict: OverrideDefaultTypesDict = {
    ...
    def map_stone_type_to_pep484_type(
    ...
    def _generate_routes(self, namespace: ApiNamespace) -> None:
    ...
    def _generate_imports_needed_for_typing(self) -> None:
        ...
        with self.capture_emitted_output(output_buffer):
            if self.import_tracker.cur_namespace_typing_imports:
                ...
                with self.indent():
                    for to_import in sorted(
                        self.import_tracker.cur_namespace_typing_imports
                    ):
                        self.emit(f"{to_import},")
                self.emit(")")
        ...
        self.add_named_placeholder(
            "imports_needed_for_typing", output_buffer.getvalue()
        )
