...
from stone.backends.python_types import class_name_for_data_type
...
_cmdline_parser.add_argument(
    ...
    type=str,
    help=(
        "The name of the Python module to generate. Please exclude the .py "
        "file extension."
    ),
)
_cmdline_parser.add_argument(
...
class PythonClientBackend(CodeBackend):
    ...
    def generate(self, api):
        ...
        with self.output_to_relative_path("%s.py" % self.args.module_name):
            ...
            with self.indent():
                ...
                self.emit("def request(self, route, namespace, arg, arg_binary=None):")
                with self.indent():
                    self.emit("pass")
                self.emit()
    ...
    def _generate_imports(self, namespaces):
        ...
        for namespace in namespaces:
            if namespace.data_types:
                self.emit(
                    "from {} import {}".format(
                        self.args.types_package, fmt_namespace(namespace.name)
                    )
                )
    ...
    def _generate_route_methods(self, namespaces):
        ...
        for namespace in namespaces:
            if namespace.routes:
                self.emit("# ------------------------------------------")
                self.emit(f"# Routes in {namespace.name} namespace")
                self.emit()
    ...
    def _generate_routes(self, namespace):
        ...
        if self.args.auth_type is not None:
            self.supported_auth_types = [
                auth_type.strip().lower()
                for auth_type in self.args.auth_type.split(",")
            ]
        ...
        for route in namespace.routes:
            ...
            if self.supported_auth_types is None:
                ...
                if route_auth_attr is None:
                    continue
                route_auth_modes = [
                    mode.strip().lower() for mode in route_auth_attr.split(",")
                ]
    ...
    def _generate_route_helper(self, namespace, route, download_to_file=False):
        ...
        if download_to_file:
            assert response_binary_body, (
                "download_to_file can only be set " "for download-style routes."
            )
            ...
            self._generate_route_method_decl(
                namespace, route, arg_data_type, request_binary_body
            )
    ...
    def _generate_route_method_decl(
            ...
            for field in arg_data_type.all_fields:
                if is_nullable_type(field.data_type):
                    ...
                    if is_user_defined_type(field.data_type):
                        ...
                        ns = None
                    arg = "{}={}".format(
                        field.name, self._generate_python_value(ns, field.default)
                    )
                    args.append(arg)
                else:
        ...
        self.generate_multiline_list(args, f"def {namespace_name}_{method_name}", ":")

    def _maybe_generate_deprecation_warning(self, route):
        if route.deprecated:
            msg = f"{route.name} is deprecated."
            if route.deprecated.by:
                msg += f" Use {route.deprecated.by.name}."
            args = [f"'{msg}'", "DeprecationWarning"]
            self.generate_multiline_list(
                args, before="warnings.warn", delim=("(", ")"), compact=False,
            )

    def _generate_docstring_for_func(
        self,
        namespace,
        arg_data_type,
        ...
        if extra_request_args or fields:
            ...
            if extra_request_args:
                for name, data_type_name, doc in extra_request_args:
                    if data_type_name:
                        field_doc = f":param {data_type_name} {name}: {doc}"
                        self.emit_wrapped_text(field_doc, subsequent_prefix="    ")
                    else:
                        self.emit_wrapped_text(
                            f":param {name}: {doc}", subsequent_prefix="    "
                        )
        ...
        if extra_return_arg:
            ...
            if is_void_type(result_data_type):
                ...
                rtype = self._format_type_in_doc(namespace, result_data_type)
                self.emit(f":rtype: {rtype}")

        if not is_void_type(error_data_type) and error_data_type.fields:
            self.emit(f":raises: :class:`{self.args.error_class_path}`")
            self.emit()
            ...
            self.emit(f"If this raises, {error_class_name} will contain:")
            with self.indent():
    ...
    def _docf(self, tag, val):
        ...
        if tag == "type":
            ...
            return f":class:`{self.args.types_package}.{fq_val}`"
        elif tag == "route":
            if ":" in val:
