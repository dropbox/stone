...
class ObjCBaseBackend(CodeBackend):
    ...
    def block_m(self, class_name):
        with self.block(f"@implementation {class_name}", delim=("", "@end"), dent=0):
            self.emit()
            yield

    @contextmanager
    def block_h_from_data_type(self, data_type, protocol=None):
        assert is_user_defined_type(
            data_type
        ...
        if not protocol:
            ...
            base = (
                fmt_class_prefix(data_type.parent_type)
                if (data_type.parent_type and not is_union_type(data_type))
                else "NSObject"
            )
            extend_suffix = " : {} <{}>".format(base, ", ".join(protocol))
        with self.block(
            "@interface {}{}".format(fmt_class_prefix(data_type), extend_suffix),
            delim=("", "@end"),
            dent=0,
        ):
            self.emit()
            ...
            if protected:
                with self.block("", delim=("", "")):
                    ...
                    for field_name, field_type in protected:
                        self.emit(f"{field_type} _{field_name};")
                self.emit("}")
            self.emit()
            yield
    ...
    def _generate_imports_h(self, import_classes):
        ...
        for import_class in import_classes:
            self.emit(f"@class {import_class};")
    ...
    def _get_namespace_route_imports(
        self, namespace, include_route_args=True, include_route_deep_args=False
    ):
        result = []
