...
_cmdline_parser.add_argument(
    "-w", "--auth-type", type=str, help="The auth type of the client to generate.",
)
_cmdline_parser.add_argument(
    "-y",
    "--client-args",
...
class ObjCBackend(ObjCBaseBackend):
    ...
    obj_name_to_namespace: typing.Dict[str, int] = {}
    namespace_to_has_routes: typing.Dict[typing.Any, bool] = {}
        ...
        for namespace in api.namespaces.values():
            if namespace.routes and self.namespace_to_has_routes[namespace]:
                import_classes = [
                    ...
                    f"{self.args.transport_client_name}Protocol",
                    "DBStoneBase",
                    "DBRequestErrors",
                ]
                ...
                with self.output_to_relative_path(
                    ...
                    self.emit(f"@protocol {self.args.transport_client_name};")
                    self.emit()
                    self._generate_routes_h(namespace)

        with self.output_to_relative_path(f"Client/{self.args.module_name}.m"):
            self._generate_client_m(api)

        with self.output_to_relative_path(f"Client/{self.args.module_name}.h"):
                        ...
                        if namespace.routes and self.namespace_to_has_routes[namespace]:
                            ...
                            self.emit(
                                base_string.format(
                                    ...
                                    fmt_routes_class(
                                        namespace.name, self.args.auth_type
                                    ),
                                )
                            )
    ...
    def _generate_client_h(self, api):
        ...
        with self.block_h(
            ...
            protected=[("transportClient", f"id<{self.args.transport_client_name}>")],
        ):
            self.emit()
            for namespace in api.namespaces.values():
                if namespace.routes and self.namespace_to_has_routes[namespace]:
            ...
            description_str = "Initializes the `{}` object with a networking client."
            self.emit_wrapped_text(
                description_str.format(self.args.class_name), prefix=comment_prefix
            )
    ...
    def _route_is_special_noauth_case(self, route):
        return (
            self._auth_type_in_route(route, "noauth") and self.args.auth_type == "user"
        )

    def _should_generate_route(self, route):
        return self._auth_type_in_route(
            route, self.args.auth_type
        ) or self._route_is_special_noauth_case(route)
            ...
            for route in namespace.routes:
                ...
                if route_type in client_args.keys():
                    for args_data in client_args[route_type]:
                        ...
                        extra_args = [
                            tuple(type_data[:-1]) for type_data in type_data_dict[1]
                        ]
    ...
    def _generate_route_m(
        self, route, namespace, route_args, extra_args, task_type_name, func_suffix
    ):
        ...
        with self.block_func(
            ...
            return_type=f"{task_type_name} *",
        ):
            self.emit(
                "DBRoute *route = {}.{};".format(
                    fmt_route_obj_class(namespace.name),
            ...
            request_call = fmt_func_call(
                caller="self.client",
                callee="request{}".format(fmt_camel_upper(route.attrs.get("style"))),
                args=fmt_func_args(transport_args),
            )
            self.emit(f"return {request_call};")
        self.emit()
    ...
    def _generate_routes_h(self, namespace):
        ...
        with self.block_h(fmt_routes_class(namespace.name, self.args.auth_type)):
            ...
            self.emit(
                fmt_property_str(
                    prop="client", typ=f"id<{self.args.transport_client_name}>"
                )
            )
            self.emit()
            ...
            init_signature = fmt_signature(
                func="init", args=routes_obj_args, return_type="instancetype"
            )
            description_str = (
    ...
    def _generate_route_signature(
        ...
        if route.doc:
            ...
            route_doc = f"The {func_name} route"
        ...
        return_type = "{}<{}, {}> *".format(
            task_type_name, result_type_str, error_type_str
        )
        ...
        self.emit(f"{route_signature}{deprecated};")
        self.emit()
    ...
    def _get_deprecation_warning(self, route):
        ...
        if route.deprecated:
            ...
            result = f' __deprecated_msg("{msg}")'
        return result

    def _get_route_args(
        self, namespace, route, tag=False
        ...
        if is_struct_type(data_type):
            ...
            doc_list = [
                (fmt_var(f.name), self.process_doc(f.doc, self._docf))
                for f in data_type.fields
                if f.doc
            ]
        elif is_union_type(data_type):
            arg_list = [
                (fmt_var(data_type.name), fmt_type(route.arg_data_type, tag=tag))
            ]
    ...
    def _get_default_route_args(
        self, namespace, route, tag=False  # pylint: disable=unused-argument
    ):
        ...
        if is_struct_type(data_type):
            ...
            doc_list = [
                (fmt_var(f.name), self.process_doc(f.doc, self._docf))
                for f in data_type.fields
                if f.doc and not f.has_default and not is_nullable_type(f.data_type)
            ]
        else:
            arg_list = []
            doc_list = []
