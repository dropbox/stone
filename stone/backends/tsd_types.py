...
from stone.backends.helpers import fmt_pascal
...
_cmdline_parser.add_argument(
    "template",
    help=(
        "A template to use when generating the TypeScript definition file. "
...
_cmdline_parser.add_argument(
    ...
    nargs="?",
    help=(
        "The name of the generated typeScript definition file that contains "
...
_cmdline_parser.add_argument(
    ...
    default=[],
    help=(
        "Additional argument to add to a route's argument based "
...
_cmdline_parser.add_argument(
    ...
    default=1,
    help=(
        "Indentation level to emit types at. Routes are automatically "
...
_cmdline_parser.add_argument(
    ...
    default="",
    help=(
        "Prefix for data type module names. "
...
class TSDTypesBackend(CodeBackend):
    ...
    def generate(self, api):
        ...
        if self.args.filename:
            ...
            for namespace in api.namespaces.values():
                filename = f"{namespace.name}.d.ts"
                self._generate_base_namespace_module(
                    [namespace],
                    filename,
                    template,
    ...
    def _generate_base_namespace_module(
        self, namespace_list, filename, template, extra_args, exclude_error_types=False
    ):
    ...
    def _generate_types(self, namespace, spaces_per_indent, extra_args):
        ...
        if self.split_by_namespace:
            generate_imports_for_referenced_namespaces(
                backend=self,
                namespace=namespace,
                module_name_prefix=self.args.module_name_prefix,
            )
        ...
        with self.indent(dent=spaces_per_indent):
            for data_type in data_types:
                self._generate_type(
                    data_type, spaces_per_indent, extra_args.get(data_type, [])
                )
    ...
    def _get_top_level_declaration(self, name):
        if self.split_by_namespace:
            ...
            return f"declare module '{self.args.module_name_prefix}{name}' {{"
        else:
    ...
    def _parse_extra_args(self, api, extra_args_raw):
        ...
        def invalid(msg, extra_arg_raw):
            print(f"Invalid --extra-arg:{msg}: {extra_arg_raw}", file=sys.stderr)
            sys.exit(1)
    ...
    def _generate_alias_type(self, alias_type):
        ...
        self.emit(
            "export type {} = {};".format(
                fmt_type_name(alias_type, namespace),
    ...
    def _generate_struct_type(self, struct_type, indent_spaces, extra_parameters):
        ...
        extends_line = (
            " extends %s" % fmt_type_name(parent_type, namespace) if parent_type else ""
        )
        self.emit(
            "export interface {}{} {{".format(
                fmt_type_name(struct_type, namespace), extends_line
            )
        )
        with self.indent(dent=indent_spaces):
            ...
            for field in struct_type.fields:
                ...
                self.emit(f"{field_name}: {field_ts_type};")
        ...
        if struct_type.is_member_of_enumerated_subtypes_tree():
            if struct_type.has_enumerated_subtypes():
                ...
                self._emit_tsdoc_header(
                    "Reference to the %s polymorphic type. Contains a .tag "
                ...
                for subtype in parent.get_enumerated_subtypes():
                    if subtype.data_type == struct_type:
                        self._emit_tsdoc_header(
                            "Reference to the %s type, identified by the "
                        ...
                        with self.indent(dent=indent_spaces):
                            self._emit_tsdoc_header(
                                "Tag identifying this subtype variant. This "
    ...
    def _generate_union_type(self, union_type, indent_spaces):
        ...
        for variant in union_type.fields:
            ...
            if is_struct_without_enumerated_subtypes:
                self.emit(
                    "export interface {} extends {} {{".format(
