...
_cmdline_parser.add_argument(
    ...
    "--route-method",
    help=(
        "A string used to construct the location of an Objective-C method for a "
...
_cmdline_parser.add_argument(
    ...
    action="store_true",
    help=(
        "Sets whether generated code should marked for exclusion " + "from analysis."
    ),
)
...
class ObjCTypesBackend(ObjCBaseBackend):
    ...
    obj_name_to_namespace: typing.Dict[str, str] = {}
    namespace_to_has_route_auth_list: typing.Dict[typing.Any, typing.Set] = {}
    ...
    def generate(self, api):
        ...
        shutil.copy(
            os.path.join(rsrc_folder, "DBStoneValidators.h"), rsrc_output_folder
        )
        shutil.copy(
            os.path.join(rsrc_folder, "DBStoneValidators.m"), rsrc_output_folder
        )
        self.logger.info("Copying DBStoneSerializers.{h,m} to output folder")
        shutil.copy(
            os.path.join(rsrc_folder, "DBStoneSerializers.h"), rsrc_output_folder
        )
        shutil.copy(
            os.path.join(rsrc_folder, "DBStoneSerializers.m"), rsrc_output_folder
        )
        self.logger.info("Copying DBStoneBase.{h,m} to output folder")
        shutil.copy(os.path.join(rsrc_folder, "DBStoneBase.h"), rsrc_output_folder)
        shutil.copy(os.path.join(rsrc_folder, "DBStoneBase.m"), rsrc_output_folder)
        self.logger.info("Copying DBSerializableProtocol.h to output folder")
        shutil.copy(
            os.path.join(rsrc_folder, "DBSerializableProtocol.h"), rsrc_output_folder
        )
        ...
        for namespace in api.namespaces.values():
            ...
            if namespace.routes:
                for route in namespace.routes:
                    auth_types = set(
                        map(lambda x: x.strip(), route.attrs.get("auth").split(","))
                    )
                    if "noauth" not in auth_types:
                        ...
                        self.namespace_to_has_route_auth_list[namespace].add("user")
        ...
        for namespace in api.namespaces.values():
            for data_type in namespace.linearize_data_types():
                self.obj_name_to_namespace[data_type.name] = fmt_class_prefix(data_type)
    ...
    def _generate_namespace_types(self, namespace, jazzy_cfg):
        ...
        for data_type in namespace.linearize_data_types():
            ...
            if self.args.documentation:
                ...
                append_to_jazzy_category_dict(
                    jazzy_cfg, "Serializers", f"{class_name}Serializer"
                )
        ...
        file_path = os.path.join(
            output_path, "DB{}Objects.m".format(fmt_camel_upper(namespace.name))
        )
        with self.output_to_relative_path(file_path):
            self.emit_raw(base_file_comment)
    ...
    def _generate_struct_class_m(self, struct):
        ...
        self._generate_imports_m(
            self._get_imports_m(
                struct, default_imports=["DBStoneSerializers", "DBStoneValidators"]
            )
        )
    ...
    def _generate_struct_class_h(self, struct):
        ...
        with self.block_h_from_data_type(
            struct, protocol=["DBSerializable", "NSCopying"]
        ):
    ...
    def _generate_union_class_m(self, union):
        ...
        self._generate_imports_m(
            self._get_imports_m(
                union, default_imports=["DBStoneSerializers", "DBStoneValidators"]
            )
        )
    ...
    def _generate_union_class_h(self, union):
        ...
        with self.block_h_from_data_type(
            union, protocol=["DBSerializable", "NSCopying"]
        ):
            self.emit("#pragma mark - Instance fields")
            self.emit()
        ...
        self.emit(comment_prefix)
        self.emit_wrapped_text(
            f"The serialization class for the `{union_name}` union.",
            prefix=comment_prefix,
        )
        self.emit(comment_prefix)
    ...
    def _generate_struct_cstor_signature(self, struct):
        ...
        for field in struct.all_fields:
            ...
            self.emit_wrapped_text(
                "@param {} {}".format(fmt_var(field.name), doc), prefix=comment_prefix
            )
    ...
    def _generate_union_cstor_funcs(self, union):
        ...
        for field in union.all_fields:
            ...
            func_args = (
                ...
                else fmt_func_args_from_fields([field])
            )
            ...
            with self.block_func(
                ...
                self.emit("self = [super init];")
                with self.block_init():
                    self.emit(f"_tag = {enum_field_name};")
                    if not is_void_type(field.data_type):
                        self.emit(
                            "_{} = {};".format(fmt_var(field.name), fmt_var(field.name))
                        )
            self.emit()

    def _generate_union_cstor_signatures(
        self, union, fields
        ...
        for field in fields:
            ...
            self.emit(comment_prefix)
            self.emit_wrapped_text(
                f'Initializes union class with tag state of "{field.name}".',
                prefix=comment_prefix,
            )
            self.emit(comment_prefix)
            if field.doc:
                ...
                self.emit_wrapped_text(
                    f'Description of the "{field.name}" tag state: {doc}',
                    prefix=comment_prefix,
                )
                self.emit(comment_prefix)
            if not is_void_type(field.data_type):
    ...
    def _generate_union_tag_state(self, union):
        ...
        description_str = (
            "The `{}` enum type represents the possible tag "
            "states with which the `{}` union can exist."
        )
        self.emit_wrapped_text(
            description_str.format(tag_type, union_name), prefix=comment_prefix
        )
        with self.block(f"typedef NS_CLOSED_ENUM(NSInteger, {tag_type})", after=";"):
            for field in union.all_fields:
                doc = (
                    self.process_doc(field.doc, self._docf)
                    if field.doc
    ...
    def _generate_serializer_signatures(self, obj_name):
        ...
        serial_signature = fmt_signature(
            func="serialize",
            args=fmt_func_args_declaration([("instance", f"{obj_name} *")]),
            return_type="nullable NSDictionary<NSString *, id>  *",
            class_func=True,
        )
        deserial_signature = fmt_signature(
            ...
            return_type=f"{obj_name} *",
            class_func=True,
        )
        self.emit(comment_prefix)
        self.emit_wrapped_text(
            f"Serializes `{obj_name}` instances.", prefix=comment_prefix
        )
        self.emit(comment_prefix)
        self.emit_wrapped_text(
            f"@param instance An instance of the `{obj_name}` API object.",
            prefix=comment_prefix,
        )
        self.emit(comment_prefix)
        description_str = (
        ...
        self.emit_wrapped_text(description_str.format(obj_name), prefix=comment_prefix)
        self.emit(comment_prefix)
        self.emit(f"{serial_signature};")
        self.emit()
        self.emit(comment_prefix)
        self.emit_wrapped_text(
            f"Deserializes `{obj_name}` instances.", prefix=comment_prefix
        )
        self.emit(comment_prefix)
        description_str = (
            "@param dict A json-compatible dictionary "
            "representation of the `{}` API object."
        )
        self.emit_wrapped_text(description_str.format(obj_name), prefix=comment_prefix)
        self.emit(comment_prefix)
        self.emit_wrapped_text(
            f"@return An instantiation of the `{obj_name}` object.",
            prefix=comment_prefix,
        )
        self.emit(comment_prefix)
        self.emit(f"{deserial_signature};")
        self.emit()
    ...
    def _generate_copyable_func(self):
        with self.block_func(
            func="copyWithZone",
            args=fmt_func_args_declaration([("zone", "NSZone *")]),
            return_type="instancetype",
        ):
    ...
    def _generate_hash_func(self, data_type):
        with self.block_func(func="hash", return_type="NSUInteger"):
            ...
            if is_union_type(data_type):
                with self.block("switch (_tag)"):
                    for field in data_type.all_fields:
                        ...
                        self.emit(f"case {enum_field_name}:")
    ...
    def _generate_hash_check(self, data_type, field):
        if is_union_type(data_type) and is_void_type(field.data_type):
            ...
            self.emit(
                "result = prime * result + [self.{} hash];".format(fmt_var(field.name))
            )
    ...
    def _generate_equality_func(self, data_type):
        ...
        with self.block_func(
            ...
            with self.block("if (!other || ![other isKindOfClass:[self class]])"):
                self.emit("return NO;")
            self.emit(f"return [self {is_equal_func_name}:other];")
        self.emit()
        ...
        with self.block_func(
            ...
            args=fmt_func_args_declaration(
                [(other_obj_name, "{} *".format(fmt_class_type(data_type)))]
            ),
            return_type="BOOL",
        ):
            with self.block(f"if (self == {other_obj_name})"):
                self.emit("return YES;")
            if is_union_type(data_type):
                with self.block(f"if (self.tag != {other_obj_name}.tag)"):
                    self.emit("return NO;")
                with self.block("switch (_tag)"):
                    for field in data_type.all_fields:
                        self._generate_equality_func_helper(
                            data_type, field, other_obj_name
                        )
                self.emit("return YES;")
            elif is_struct_type(data_type):
                for field in data_type.all_fields:
    ...
    def _generate_equality_check(self, data_type, field, other_obj_name):
        ...
        if is_union_type(data_type):
            if is_void_type(field.data_type):
                self.emit(
                    f"return [[self tagName] isEqual:[{other_obj_name} tagName]];"
                )
            else:
                self.emit(
                    "return [self.{0} isEqual:{1}.{0}];".format(
                        field_name, other_obj_name
                    )
                )
        elif is_struct_type(data_type):
            with self.block(
                "if (![self.{0} isEqual:{1}.{0}])".format(field_name, other_obj_name)
            ):
                self.emit("return NO;")
    ...
    def _cstor_args_from_fields(self, fields, is_struct=False):
        ...
        if is_struct:
            args = [
                (
                    fmt_var(f.name),
                    fmt_type(f.data_type, tag=True, has_default=f.has_default),
                )
                for f in fields
            ]
        else:
    ...
    def _generate_validator(self, field):
        ...
        validator = self._determine_validator_type(
            field.data_type, fmt_var(field.name), field.has_default
        )
        value = (
            fmt_var(field.name)
            if not field.has_default
            else "{} ?: {}".format(fmt_var(field.name), fmt_default_value(field))
        )
        if validator:
            self.emit(f"{validator}({value});")
    ...
    def _determine_validator_type(self, data_type, value, has_default):
        ...
        if is_list_type(data_type):
            ...
            validator = "{}:{}".format(
                ...
                fmt_func_args(
                    [
                        (
                            "minItems",
                            f"@({data_type.min_items})"
                            if data_type.min_items
                            else "nil",
                        ),
                        (
                            "maxItems",
                            f"@({data_type.max_items})"
                            if data_type.max_items
                            else "nil",
                        ),
            ...
            validator = "{}:{}".format(
                fmt_validator(data_type),
                fmt_func_args([("itemValidator", item_validator)]),
            )
                ...
                validator = "{}:{}".format(
                    ...
                    fmt_func_args(
                        [
                            (
                                "minValue",
                                f"@({data_type.min_value})"
                                if data_type.min_value
                                else "nil",
                            ),
                            (
                                "maxValue",
                                f"@({data_type.max_value})"
            ...
            if data_type.pattern or data_type.min_length or data_type.max_length:
                pattern = (
                    data_type.pattern.encode("unicode_escape").replace('"', '\\"')
                ...
                validator = "{}:{}".format(
                    ...
                    fmt_func_args(
                        [
                            ...
                            ("pattern", f'@"{pattern}"' if pattern else "nil"),
                        ]
                    ),
                )
        ...
        if nullable:
            if validator:
                validator = fmt_func_call(caller="DBStoneValidators", callee=validator)
    ...
    def _generate_struct_serializer(self, struct):
        ...
        with self.block_func(
            func="serialize",
            args=fmt_func_args_declaration([("valueObj", f"{struct_name} *")]),
            ...
            for field in struct.all_fields:
                ...
                if not nullable:
                    if is_primitive_type(data_type):
                        self.emit(f'jsonDict[@"{field.name}"] = {input_value};')
                    else:
                        self.emit(f'jsonDict[@"{field.name}"] = {serialize_call};')
                else:
                    with self.block(f"if ({input_value})"):
                        self.emit(f'jsonDict[@"{field.name}"] = {serialize_call};')
            self.emit()
                    ...
                    with self.block(
                        ...
                        func_args = fmt_func_args(
                            [
                                (
                                    "value",
                                    "({} *)valueObj".format(fmt_class_prefix(subtype)),
                                )
                            ]
                        )
                        ...
                        self.emit(f"NSDictionary *subTypeFields = {serialize_call};")
    ...
    def _generate_struct_deserializer(self, struct):
        ...
        with self.block_func(
            func="deserialize",
            args=fmt_func_args_declaration(
                [("valueDict", "NSDictionary<NSString *, id> *")]
            ),
            return_type=f"{struct_name} *",
            class_func=True,
        ):
            if not struct.all_fields and not struct.has_enumerated_subtypes():
            ...
            def emit_struct_deserialize_logic(struct):
                for field in struct.all_fields:
                    data_type, nullable = unwrap_nullable(field.data_type)
                    input_value = f'valueDict[@"{field.name}"]'
                    ...
                    if nullable or field.has_default:
                        default_value = (
                            fmt_default_value(field) if field.has_default else "nil"
                        )
                        if is_primitive_type(data_type):
            ...
            if not struct.has_enumerated_subtypes():
                ...
                for tags, subtype in struct.get_all_subtypes_with_tags():
                    ...
                    with self.block(base_string.format(tag)):
                        ...
                        self.emit(f"return {deserialize_call};")
    ...
    def _generate_union_serializer(self, union):
        ...
        with self.block_func(
            func="serialize",
            args=fmt_func_args_declaration([("valueObj", f"{union_name} *")]),
            return_type="NSDictionary<NSString *, id> *",
            class_func=True,
        ):
            ...
            for field in union.all_fields:
                with self.block(
                    "{} ([valueObj is{}])".format(
                        "if" if first_block else "else if", fmt_camel_upper(field.name)
                    )
                ):
                    data_type, nullable = unwrap_nullable(field.data_type)
                    input_value = "valueObj.{}".format(fmt_var(field.name))
                    ...
                    if not is_void_type(data_type):
                        if not nullable:
                            ...
                            with self.block(
                                "if (valueObj.{})".format(fmt_var(field.name))
                            ):
                                emit_serializer()

                    self.emit(f'jsonDict[@".tag"] = @"{field.name}";')
    ...
    def _generate_union_deserializer(self, union):
        ...
        with self.block_func(
            ...
            for field in union.all_fields:
                ...
                with self.block(
                    base_cond.format("if" if first_block else "else if", field.name)
                ):
                    if first_block:
                        first_block = False
                    if not is_void_type(field.data_type):
    ...
    def _fmt_serialization_call(self, data_type, input_value, serialize, depth=0):
        ...
        if is_list_type(data_type) or is_map_type(data_type):
            ...
            serialization_call = self._fmt_serialization_call(
                elem_data_type, f"elem{depth}", serialize, depth + 1
            )
            ...
            serializer_args.append(("value", input_value))
            serializer_args.append(("dateFormat", f'@"{data_type.format}"'))
        else:
            serializer_args.append(("value", input_value))
    ...
    def _generate_route_objects_m(self, route_schema, namespace):
        ...
        with self.output_to_relative_path(output_path):
            ...
            imports_classes_m = import_classes + self._get_imports_m(
            ...
            with self.block_m(fmt_route_obj_class(namespace.name)):
                for route in namespace.routes:
                    ...
                    self.emit(f"static DBRoute *{route_name};")
                self.emit()
                ...
                for route in namespace.routes:
                    ...
                    if route.version == 1:
                        ...
                        route_path = f"{route.name}_v{route.version}"
                    ...
                    if not is_void_type(route.result_data_type):
                        ...
                        result_type = fmt_func_call(caller=caller, callee="class")
                    else:
                        result_type = "nil"
                    ...
                    if not is_void_type(route.error_data_type):
                        ...
                        error_type = fmt_func_call(caller=caller, callee="class")
                    else:
                        error_type = "nil"

                    if is_list_type(route.arg_data_type) or is_map_type(
                        route.arg_data_type
                    ):
                    ...
                    if is_list_type(route.result_data_type) or is_map_type(
                    ...
                    with self.block_func(
                        ...
                        with self.block(f"if (!{route_name})"):
                            with self.block(
                                f"{route_name} = [[DBRoute alloc] init:",
                                delim=(None, None),
                                after="];",
                            ):
                                self.emit(f'@"{route_path}"')
                                self.emit(f'namespace_:@"{namespace.name}"')
                                self.emit(f"deprecated:{deprecated}")
                                self.emit(f"resultType:{result_type}")
                                self.emit(f"errorType:{error_type}")
                                ...
                                self.emit(
                                    "dataStructSerialBlock:{}".format(
                                        dataStructSerialBlock
                                    )
                                )
                                self.emit(
                                    "dataStructDeserialBlock:{}".format(
                                        dataStructDeserialBlock
                                    )
                                )

                        self.emit(f"return {route_name};")
                    self.emit()

    def _generate_route_objects_h(
        self, route_schema, namespace  # pylint: disable=unused-argument
    ):
        ...
        with self.output_to_relative_path(output_path):
            ...
            with self.block_h(fmt_route_obj_class(namespace.name)):
                for route in namespace.routes:
                    ...
                    self.emit_wrapped_text(
                        base_str.format(fmt_route_func(route)), prefix=comment_prefix
                    )
                    self.emit(f"{route_obj_access_signature};")
                    self.emit()
    ...
    def _generate_union_tag_access_signatures(self, union):
        ...
        for field in union.all_fields:
            ...
            if not is_void_type(field.data_type):
                ...
                self.emit_wrapped_text(
                    warning_str.format(fmt_var(field.name)), prefix=comment_prefix
                )
                self.emit(comment_prefix)
        ...
        self.emit(comment_prefix)
        self.emit(f"{get_tag_name_signature};")
        self.emit()
    ...
    def _generate_union_tag_state_funcs(self, union):
        ...
        for field in union.all_fields:
            ...
            with self.block_func(
                ...
                self.emit(f"return _tag == {enum_field_name};")
            self.emit()

        with self.block_func(func="tagName", args=[], return_type="NSString *"):
            with self.block("switch (_tag)"):
                for field in union.all_fields:
                    enum_field_name = fmt_enum_name(field.name, union)
                    self.emit(f"case {enum_field_name}:")
                    self.emit(f'   return @"{enum_field_name}";')
            self.emit()
            self._generate_throw_error("InvalidTag", '@"Tag has an unknown value."')
        self.emit()
        ...
        for field in union.all_fields:
            if not is_void_type(field.data_type):
                ...
                with self.block_func(
                    ...
                    with self.block(
                        "if (![self is{}])".format(fmt_camel_upper(field.name)),
                        delim=("{", "}"),
                    ):
    ...
    def _generate_union_tag_property(self, union):
        ...
        self.emit_wrapped_text(
            "Represents the union's current tag state.", prefix=comment_prefix
        )
        self.emit(
            fmt_property_str(prop="tag", typ="{}".format(fmt_enum_name("tag", union)))
        )
        self.emit()
    ...
    def _generate_class_comment(self, data_type):
        ...
        if data_type.doc:
            self.emit(comment_prefix)
            self.emit_wrapped_text(
                self.process_doc(data_type.doc, self._docf), prefix=comment_prefix
            )
