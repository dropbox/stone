...
class Attribute(object):
    ...
    def __get__(self, instance, owner):
        ...
        raise AttributeError(
            "missing required field '{}'".format(public_name(self.name))
        )
...
class Struct(object):
    ...
    def __repr__(self):
        args = [
            "{}={!r}".format(name, getattr(self, "_{}_value".format(name)))
...
class Union(object):
    ...
    def __init__(self, tag, value=None):
        validator = None
        tagmap_names = [
            "_{}_tagmap".format(map_name) for map_name in self._permissioned_tagmaps
        ]
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and (isinstance(self, other.__class__) or isinstance(other, self.__class__))
...
class Route(object):
    ...
    def __init__(
        self, name, version, deprecated, arg_type, result_type, error_type, attrs
    ):
        self.name = name
...
def make_list_annotation_processor(processor):
    def g(field_path, list_):
        if list_ is None:
            return list_
        return [
            processor("{}[{}]".format(field_path, idx), x)
...
def make_map_value_annotation_processor(processor):
    def g(field_path, map_):
        if map_ is None:
            return map_
        return {
            k: processor("{}[{}]".format(field_path, repr(k)), v)
