...
def fmt_serial_class(class_name):
    return f"{class_name}Serializer"
...
def fmt_route_var(namespace_name, route):
    ret = "DB{}{}".format(fmt_class_caps(namespace_name), fmt_camel_upper(route.name))
    if route.version != 1:
        ret = f"{ret}V{route.version}"
...
def fmt_func_args(arg_str_pairs):
    ...
    for arg_name, arg_value in arg_str_pairs:
        if first_arg:
            result.append(f"{arg_value}")
            first_arg = False
        else:
            result.append(f"{arg_name}:{arg_value}")
    return " ".join(result)
...
def fmt_func_args_declaration(arg_str_pairs):
    ...
    for arg_name, arg_type in arg_str_pairs:
        if first_arg:
            result.append(f"({arg_type}){arg_name}")
            first_arg = False
        else:
...
def fmt_signature(func, args, return_type="void", class_func=False):
    ...
    if args:
        result = f"{modifier} ({return_type}){func}:{args}"
    else:
        result = f"{modifier} ({return_type}){func}"
