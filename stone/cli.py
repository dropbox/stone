...
from .compiler import BackendException, Compiler
...
_cmdline_parser.add_argument(
    "-v", "--verbose", action="count", help="Print debugging statements.",
)
_backend_help = (
...
_cmdline_parser.add_argument(
    "backend", type=str, help=_backend_help,
)
_cmdline_parser.add_argument(
    "output", type=str, help="The folder to save generated files to.",
)
_cmdline_parser.add_argument(
    "spec",
    nargs="*",
    type=str,
    help=(
        "Path to API specifications. Each must have a .stone extension. "
...
_cmdline_parser.add_argument(
    ...
    type=str,
    help=(
        "Removes routes that do not match the expression. The expression "
...
_cmdline_parser.add_argument(
    ...
    default=[],
    help=(
        "Route attributes that the backend will have access to and "
...
def main():
    ...
    if args.spec and args.spec[0].startswith("+") and args.spec[0].endswith(".py"):
        ...
        try:
            api = imp.load_source(  # pylint: disable=redefined-outer-name
                "api", args.api[0]
            ).api
        except ImportError as e:
            print("error: Could not import API description due to:", e, file=sys.stderr)
            sys.exit(1)
    else:
        if args.spec:
            specs = []
            ...
            for spec_path in args.spec:
                if spec_path == "-":
                    ...
                    print(
                        "error: Specification '%s' cannot be found." % spec_path,
                        file=sys.stderr,
                    )
                    sys.exit(1)
                else:
        ...
        if not args.spec or read_from_stdin:
            ...
            if len(parts) == 1:
                ...
                while parts:
                    specs.append(
                        ("stdin.%s" % (len(specs) + 1), "namespace%s" % parts.pop(0))
                    )
        ...
        try:
            # TODO: Needs version
            api = specs_to_ir(
                specs, debug=debug, route_whitelist_filter=route_whitelist_filter
            )
        ...
        if attrs:
            attr = attrs.pop()
            print(
                "error: Attribute not defined in stone_cfg.Route: %s" % attr,
                file=sys.stderr,
            )
            sys.exit(1)

    if args.backend in _builtin_backends:
        backend_module = __import__("stone.backends.%s" % args.backend, fromlist=[""])
    elif not os.path.exists(args.backend):
        print("error: Backend '%s' cannot be found." % args.backend, file=sys.stderr)
        sys.exit(1)
    elif not os.path.isfile(args.backend):
        print("error: Backend '%s' must be a file." % args.backend, file=sys.stderr)
        sys.exit(1)
    elif not Compiler.is_stone_backend(args.backend):
        print(
            "error: Backend '%s' must have a .stoneg.py extension." % args.backend,
            file=sys.stderr,
        )
        sys.exit(1)
    else:
        ...
        try:
            ...
            print(
                "error: Importing backend '%s' module raised an exception:"
                % args.backend,
                file=sys.stderr,
            )
            raise
    ...
    c = Compiler(
        api, backend_module, backend_args, args.output, clean_build=args.clean_build,
    )
    try:
        c.build()
